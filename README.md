...
- **最小权限策略引擎**：声明式规则（工具名 + 参数模式）裁决 allow/deny/ask；`ask` 经工作台弹窗人工审批（允许一次/拒绝），无回调时默认拒绝（fail closed）
...
- **流式对话**：打字机输出、运行中可停止
- **多工作区隔离**：侧边栏「工作区 → 管理」可新建/切换/删除多个工作区 —— 每个工作区独立目录（agent 读写与命令锚定其中）**且独立对话历史**，在不同工作区执行不同任务互不干扰，元数据持久化重启保留
