...
1. **轨迹记录与离线重放**：一次运行的完整事件流固化为 JSONL 轨迹 → 离线重放校验事件顺序、复算工具序列与 token，黄金轨迹比对检出行为漂移 → **让不可预测的 agent 行为可回归测试**（CI 可接入）—— 可测
2. **Token 预算驱动的上下文压缩**：超预算自动分层压缩（先裁剪超长工具结果、再折叠最旧轮次为结构化摘要）→ **长会话 token 峰值实测 ↓96%**，成本可控 —— 可控
3. **参数级最小权限策略 + 人工审批流**：按"工具名 + 参数模式"裁决 allow/deny/ask，危险命令（如 `rm -rf`）直接拦截、高风险调用弹窗审批、无审批回调默认拒绝 → **危险操作可控**（fail-closed）—— 可控
4. **技能学习（经验沉淀闭环）**：把成功运行的轨迹自动提炼为可复用 SKILL.md（多条同类轨迹按采用频次加权）→ **跑成功即沉淀，下次同类任务自动复用**，用越多越聪明 —— 可成长
