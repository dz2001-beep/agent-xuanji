...
**璇玑（xuanji）** 是一个从零实现的 TypeScript **Agent Harness**（代理运行时）：把 **Agent Loop（循环编排）、MCP（工具协议）、Skill（技能系统）** 组合成带 Web 工作台的完整产品，并针对 agent 落地难题做了四项可靠性优化。支持：循环状态机（5 种终止态、重试/取消/超时、参数校验、类型化事件流）、MCP 接入（stdio/HTTP、多 server）、SKILL.md 技能系统（中英文匹配、自动注入 + 技能学习）、Codex 式工作台（SSE 流式对话、工作区隔离、工具可视化、模型切换、审批弹窗），以及免 Key 的真实天气（IP 定位/我的城市）与联网搜索。GitHub：github.com/dz2001-beep/agent-xuanji

### 四项优化（做什么 → 解决什么问题）

1. **轨迹记录与离线重放**：一次运行的完整事件流固化为 JSONL 轨迹 → 离线重放校验事件顺序、复算工具序列与 token，黄金轨迹比对检出行为漂移 → **让不可预测的 agent 行为可回归测试**（CI 可接入）
2. **Token 预算驱动的上下文压缩**：超预算自动分层压缩（先裁剪超长工具结果、再折叠最旧轮次为结构化摘要）→ **长会话 token 峰值实测 ↓96%**，成本可控
3. **参数级最小权限策略 + 人工审批流**：按"工具名 + 参数模式"裁决 allow/deny/ask，危险命令（如 `rm -rf`）直接拦截、高风险调用弹窗审批、无审批回调默认拒绝 → **危险操作可控**（fail-closed）
4. **技能学习（经验沉淀闭环）**：把成功运行的轨迹自动提炼为可复用 SKILL.md（多条同类轨迹按采用频次加权）→ **跑成功即沉淀，下次同类任务自动复用**，用越多越聪明
