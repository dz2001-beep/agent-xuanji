...
2. **Token 预算驱动的上下文压缩**：在 Agent Loop 模型调用前接入可配置的预算检查，超出预算时按层级自动压缩：先截断超长工具结果，再把最旧的对话轮次折叠为保留工具序列与结论的结构化摘要；压缩过程不依赖 LLM（可离线确定性测试），压缩前后 token 数与裁剪/折叠次数通过事件流可观测；实测长会话 token 由 29,990 压缩至 1,240（减少约 96%）
